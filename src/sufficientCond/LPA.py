...
class LPA:
    ...
    def testPriority(self, i, p):
        ...
        for j in range(self.tasks.getSize()):
            task = self.tasks[j]
            res += self.nbJobs[j] * task[self.tasks[self.instance[i]]["X"]]

        rhs = (self.nbJobs[self.instance[i]] - 1) * self.tasks[self.instance[i]][
            "T"
        ] + self.tasks[self.instance[i]]["D"]
        return res <= rhs
    ...
    def initAssign(self):
        ...
        for i in range(self.tasks.getSize()):
            ...
            nbjobsPerTask = BP // task["T"] + int(BP % task["T"] > 0)
            for j in range(int(nbjobsPerTask)):
    ...
    def computeGamma(self, X):
        ...
        for i in range(self.tasks.getSize()):
            if self.tasks[i]["X"] >= X:
                sumCHigherX += self.tasks[i][X]
                sumConTHigherX += (self.tasks[i][X] * 1.0) / self.tasks[i]["T"]
        ...
        for i in range(self.tasks.getSize()):
            if self.tasks[i]["X"] == X:
                returnSum += self.tasks[i][X] * (
                    1 + math.floor(gamma / self.tasks[i]["T"])
                )
        res = self.Ld[X] + returnSum
        self.Ld[X] = res
        return res
