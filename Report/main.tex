...
\begin{defin}[Transition de terminaison]
Soit $S = (nat_S, rct_S, done_S, crit_S)$ un état du système et $\tau' \subseteq Run$ un ensemble de tâches actives pouvant signaler leur complétion. On dit que l'état du système $S' = (nat_S', rct_S', done_S', crit_S')$ est un $successeur\ \tau'-terminaison$ de $S$, noté $S\xrightarrow{\tau'}S'$ ssi:
\begin{itemize}
\item Pour tout $\tau_i \in \tau' \cup ImplicitelyDone(S)$ :\begin{itemize}
\item $rct_{S'}(\tau_i) = 0$
\item $nat_{S'}(\tau_i) = nat_{S}(\tau_i)$
\item $done_{S'}(\tau_i) = True$
\end{itemize}

\item Pour tout $\tau_i\ not\ \in \tau' \cup ImplicitelyDone(S)$ :\begin{itemize}
\item $rct_{S'}(\tau_i) = rct_{S}(\tau_i)$
\item $nat_{S'}(\tau_i) = nat_{S}(\tau_i)$
\item $done_{S'}(\tau_i) = done_{S}(\tau_i)$
\end{itemize}
\item $crit_{S'} = crit_{S}$
...
Soit $S = (nat_S, rct_S, done_S, crit_S)$ un état du système et $\tau' \subseteq Eligible(S)$ un ensemble de tâches eligible. On dit que l'état du système $S^R = (nat_S^R, rct_S^R, done_S^R, crit_S^R)$ est un $successeur\ \tau'-request$ de $S$, noté $S\xrightarrow{\tau'}S^R$ ssi:

\begin{itemize}
\item Pour tout $\tau_i \in \tau'$ :\begin{itemize}
    \item $nat_S(\tau_i)+T_i \leq nat_S^R(\tau_i) \leq T_i$
    \item $rct_S^R(\tau_i)=C_i(crit_S)$
    \item $done_S^R(\tau_i) = False$
\end{itemize}
\item Pour tout $\tau_i\ not \in \tau'$ :\begin{itemize}
...
\begin{itemize}
\item  $V=State(\tau)$
\item $(S_1,S_2) \in E$ ssi il existe les états intermédiaires $S', S''$ et $S''' \in State(\tau)$ et $\tau' \subseteq Active(S') $ tel que : $S_1\xrightarrow{Run}S'\xrightarrow{\tau'}S''\xrightarrow{C}S_2$
\item $S_0 = (at_{S_0}, rct_{S_0}, crit_{S_0})$ :\begin{itemize}
\item $at_{S_0} = O_i\ \forall \tau_i \in \tau$
\item $rct_{S_0} = C_i(1)\ \forall \tau_i \in \tau$
\item $crit_{S_0} = 1$
