...
class SystemState:
    ...
    def getWorstLaxity(self, C):
        ...
        for i in range(len(self.nat)):
            laxity.append(self.nat[i]-(self.rct[i] +(C[i][-1]-C[i][self.crit-1])*(not self.done[i]) )+D[i]-T[i])
        return laxity
...
#            if self.rct[i] == 0 and self.crit == X[i]:
    ...
    def getExecutionTransition(self, run, X):
        ...
        return SystemState(natP, rctP, doneP, critP, self.SIM, self.hashInfo, self.scheduler)
    ...
    def getRequestTransition(self, request, C):
        ...
        for i in range(nbCombinaison):
            ...
            res.append(SystemState(list(natP), list(rctP), list(doneP), critP, self.SIM, self.hashInfo, self.scheduler))
        return res
    ...
    def isSimulation(self, ss):
        if self.SIM == IDLE:
            ...
            return (self.nat == ss.nat and self.rct == ss.rct and self.crit == ss.crit and self.done == ss.done)

    def isIdleSimulation(self, ss):
        fullRes = map(int.__le__, self.getRelevantAttributeIdle(), ss.getRelevantAttributeIdle())
        res = reduce(bool.__and__, fullRes, True)
        return res

    def isIdleWCSimulation(self, ss):
        resIdle = map(int.__le__, self.getRelevantAttributeIdle(), ss.getRelevantAttributeIdle())
        resWC1 = map(int.__ge__, self.getRelevantAttributeWC(), ss.getRelevantAttributeWC())
    ...
    def getRelevantAttributeWC(self):
        return tuple(self.rct[i] for i in range(len(self.rct)) if ( (not self.done[i]) and self.rct[i] > 0))

    def getRelevantAttributeIdleWC(self):
        return (tuple(self.nat[i] for i in range(len(self.nat)) if self.done[i]), tuple(self.rct[i] for i in range(len(self.rct)) if ( (not self.done[i]) and self.rct[i] > 0)))



    def __repr__(self):
        #return("SS#"+str(hash(self))+"\n nat :"+str(self.nat)+"\n rct :"+str(self.rct)+"\n done :"+str(bool(self.done))+"\n crit :"+str(self.crit))
        return('\n'+str(self.nat)+", "+str(self.rct)+", "+str(list(map(bool,self.done)))+", "+str(self.crit))
